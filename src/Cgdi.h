...
public:
  ...
  void TextAtPos(Vec2 pos, const std::string &s)
  {
    TextOutA(m_hdc, (int)pos.mX, (int)pos.mY, s.c_str(), (int)s.size());
  }
  ...
  //----------------------------pixels
  void DrawDot(Vec2 pos, COLORREF color)
  {
    SetPixel(m_hdc, (int)pos.mX, (int)pos.mY, color);
  }
  ...
  {
    MoveToEx(m_hdc, (int)from.mX, (int)from.mY, NULL);
    LineTo(m_hdc, (int)to.mX, (int)to.mY);
  }
  ...
  void PolyLine(const std::vector<Vec2>& points)
  {
    //make sure we have at least 2 points
    if (points.size() < 2) return;

    MoveToEx(m_hdc, (int)points[0].mX, (int)points[0].mY, NULL);

    for (unsigned int p=1; p<points.size(); ++p)
    {
      LineTo(m_hdc, (int)points[p].mX, (int)points[p].mY);
    }
  }

  //void LineWithArrow(Vec2 from, Vec2 to, double size)
  //{
  //  Vec2 norm = Vec2DNormalize(to-from);

  //  //calculate where the arrow is attached
  //  Vec2 CrossingPoint = to - (norm * size);
  //  
  //  //calculate the two extra points required to make the arrowhead
  //  Vec2 ArrowPoint1 = CrossingPoint + (norm.GetPerpendicular() * 0.4f * size); 
  //  Vec2 ArrowPoint2 = CrossingPoint - (norm.GetPerpendicular() * 0.4f * size); 
  ...
  void Cross(Vec2 pos, int diameter)
  {
    Line((int)pos.mX-diameter, (int)pos.mY -diameter, (int)pos.mX+diameter, (int)pos.mY+diameter);
    Line((int)pos.mX-diameter,(int)pos.mY+diameter, (int)pos.mX+diameter, (int)pos.mY-diameter);
  }
  ...
  void ClosedShape(const std::vector<Vec2> &points)
  {
    MoveToEx(m_hdc, (int)points[0].mX, (int)points[0].mY, NULL);
    ...
    LineTo(m_hdc, (int)points[0].mX, (int)points[0].mY);
  }


  void Circle(Vec2 pos, double radius)
  {
    Ellipse(m_hdc,
           (int)(pos.mX-radius),
           (int)(pos.mY-radius),
